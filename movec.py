...
def moveRobotToPoint(robot_ip, points, nexpoints):
    conSuc, sock = connectETController(robot_ip)
    ...
    try:
        ...
        P000 = points
        P001 = nexpoints
        print("Moving to point:", P000)
        print("Target point:", P001)

        # Calculate the intermediate position (midPos) for creating a circular arc
        P000 = [(p + n) / 2 for p, n in zip(points[0], nexpoints[0])]
    ...
    if con_success:
        try:
            for pose in offset_poses:
                ...
                jointlist_pick1 = inverseKinematicSolver(robot_ip, [pickup_point])
                jointlist_pick2 = inverseKinematicSolver(robot_ip, [transfer_point])
                moveRobotToPoint(robot_ip, jointlist_pick1,jointlist_pick2)
                
                # Move to the pickup point first
                jointlist_transfer1 = inverseKinematicSolver(robot_ip, [transfer_point])
                jointlist_transfer2 = inverseKinematicSolver(robot_ip, [pose])
                moveRobotToPoint(robot_ip,jointlist_transfer1,jointlist_transfer2)

                # Then move to the current target pose
                jointlist_place1 = inverseKinematicSolver(robot_ip, [pose])  # Pass pose as a list
                jointlist_place2 = inverseKinematicSolver(robot_ip, [pickup_point])
                moveRobotToPoint(robot_ip, jointlist_place1,jointlist_place2)  # Extract the first element from the list
